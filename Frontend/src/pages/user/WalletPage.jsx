...
export default function WalletPage() {
  ...
  if (loading) return <div className="flex items-center justify-center h-64 text-gray-400">Loading wallet...</div>;
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="card bg-gradient-to-br from-fresh-900/40 to-gray-900 border-fresh-800/50 glow-green">
          ...
          {parseFloat(wallet?.due_amount || 0) > 0 && (
            <div className="mt-3 flex items-center gap-2">
              ...
              <span className="text-red-400 text-sm">₹{parseFloat(wallet.due_amount).toFixed(2)} pending</span>
            </div>
          )}
        </div>
        ...
        {/* Add funds */}
        <div className="card">
          <h3 className="font-semibold text-white mb-4">Add Funds</h3>
              ...
              <tbody>
                {transactions.map(tx => (
                  <tr key={tx.id} className="table-row">
                    <td className="p-3 text-gray-400">{new Date(tx.created_at).toLocaleDateString("en-IN")}</td>
                    <td className="p-3 text-gray-300">{tx.reason}</td>
                    <td className="p-3 text-right">
                      <span className={tx.type === "credit" ? "badge-green" : "badge-red"}>{tx.type}</span>
                    </td>
                    <td className={`p-3 text-right font-semibold ${tx.type === "credit" ? "text-fresh-400" : "text-red-400"}`}>
