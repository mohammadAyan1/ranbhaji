...
export default function ItemPurchaseReport() {
    ...
    const fetchReport = async () => {
        ...
        setLoading(true);
        try {
            const res = await api.get(`/reports/items-purchased?from=${from}&to=${to}`);
    ...
    return (
        <div className="p-6 max-w-6xl mx-auto space-y-6">
            <div className="flex flex-col md:flex-row items-center justify-between bg-white p-6 rounded-2xl shadow-sm border border-gray-100 gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900 tracking-tight">Total Item Purchase</h1>
                    <p className="text-gray-500 text-sm mt-1">Report of total items purchased within a date range.</p>
                </div>
                            ...
                            {loading ? (
                                ...
                                filteredData.map((item, idx) => (
                                    <tr key={idx} className="hover:bg-gray-50/50 transition-colors">
                                        ...
                                        <td className="px-6 py-4 text-gray-600">{item.total_quantity} {item.product?.unit}</td>
                                        <td className="px-6 py-4 text-gray-600">₹{item.total_amount?.toFixed(2)}</td>
                                    </tr>
                                ))
                            )}
                        </tbody>
