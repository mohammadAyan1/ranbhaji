...
export default function ReturnItem() {
  ...
  const fetchReturns = async () => {
    ...
    try {
      ...
      (res.data.returns || []).forEach(r => {
        if (r.is_full_return) return; // Only show PARTIAL returns here
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div>
        <h1 className="page-header">Return Item Queue 📦</h1>
        <p className="page-sub">Review and process partial item return requests</p>
      </div>
      ...
      {returns.length === 0 ? (
        <div className="card text-center py-16 text-gray-500">
          ...
          <p>No item return requests are in the queue.</p>
        </div>
      ) : (
