            ...
            <tbody>
              {users.map(u => (
                <tr key={u.id} className="table-row">
                  ...
                  <td className="p-3 text-right text-fresh-400">₹{parseFloat(u.wallet_balance || 0).toFixed(0)}</td>
                  <td className="p-3 text-right text-red-400">₹{parseFloat(u.due_amount || 0).toFixed(0)}</td>
                  <td className="p-3 text-center">
                    <span className={u.status === "active" ? "badge-green" : "badge-red"}>{u.status}</span>
                  </td>
                  <td className="p-3 text-right">
                    <button onClick={() => toggleStatus(u)}
                      className={`text-xs font-medium ${u.status === "active" ? "text-red-400 hover:text-red-300" : "text-fresh-400 hover:text-fresh-300"}`}>
