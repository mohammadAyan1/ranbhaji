...
export default function RetailCustomers() {
  ...
  useEffect(() => {
    api.get("/admin/user-analytics/customers-filtered?tab=retail")
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      <div className="flex flex-col md:flex-row items-center justify-between bg-white p-6 rounded-2xl shadow-sm border border-gray-100 gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 tracking-tight">Retail Customers</h1>
          <p className="text-gray-500 text-sm mt-1">Customers with 0 active package but 1+ retail order.</p>
        </div>
        <div className="flex items-center gap-4 w-full md:w-auto">
          <div className="relative flex-1 md:w-64">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 border border-gray-200 rounded-xl focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500 transition-all text-sm"
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
            />
          </div>
          <div className="bg-amber-50 text-amber-700 px-4 py-2 rounded-xl font-bold border border-amber-100 whitespace-nowrap">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center text-gray-400 font-medium">No retail customers found.</div>
        ) : (
