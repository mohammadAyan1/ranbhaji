...
export default function AdminDashboard() {
  ...
  return (
    <div className="space-y-6 sm:space-y-8 animate-fade-in max-w-7xl mx-auto">
      <div>
        <h1 className="page-header flex items-center gap-2">Admin Dashboard <ShieldAlert className="text-red-400" size={28} /></h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        {[
          ...
          { label: "Active Users", value: users.filter(u => u.status === "active").length, icon: <CheckCircle size={24} />, color: "bg-fresh-500/20 text-fresh-400", cardBorder: "hover:border-fresh-500/50 hover:shadow-fresh-900/20" },
          ...
          { label: "Admins", value: users.filter(u => u.role === "admin").length, icon: <ShieldAlert size={24} />, color: "bg-red-500/20 text-red-400", cardBorder: "hover:border-red-500/50 hover:shadow-red-900/20" },
        ].map(stat => (
        ...
        <div className="card-glass">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-lg font-bold text-white tracking-tight flex items-center gap-2">
              ...
              <span className="text-sm font-normal text-gray-400">({summary?.date})</span>
            </h2>
          </div>
                ...
                <tbody>
                  {summary.deliveries.map(d => (
                    <tr key={d.schedule_id} className="table-row">
                      <td className="p-4 text-white font-medium">{d.user?.name}</td>
                      <td className="p-4 text-gray-400">
                        <span className="bg-gray-800 px-2 py-1 rounded-md text-xs border border-gray-700">{d.package}</span>
                      </td>
              ...
              <tbody>
                {users.slice(0, 8).map(u => (
                  <tr key={u.id} className="table-row">
                    ...
                    <td className="p-4 text-right text-gray-300 font-medium tracking-tight">₹{parseFloat(u.wallet_balance || 0).toFixed(0)}</td>
