              ...
              {users.map(u => {
                ...
                return (
                  <tr key={u.id} className="table-row">
                    <td className="p-3 text-white font-medium align-top">{u.name}</td>
                    <td className="p-3 text-gray-400 align-top">{u.phone}</td>
                    ...
                    <td className="p-3 text-gray-300 align-top">
                      {addresses.length === 0 ? (
                        ...
                        <ul className="space-y-2">
                          {addresses.map(addr => (
                            <li key={addr.id} className="pb-2 border-b border-gray-800 last:border-0 last:pb-0">
                              <p className="font-medium text-white">{addr.address_line}</p>
                              ...
                              {addr.is_default && <span className="text-[10px] bg-fresh-900 text-fresh-400 px-2 py-0.5 rounded-full mt-1 inline-block">Default</span>}
                            </li>
                          ))}
                        </ul>
                      )}
                                ...
                                {addr.latitude && addr.longitude ? (
                                    <div className="flex items-center gap-2">
                                        <a 
                                            ...
                                            target="_blank" 
                                            rel="noreferrer"
                                            className="text-fresh-400 hover:underline"
                                        >
                                            {addr.latitude}, {addr.longitude}
                                        </a>
                                    </div>
